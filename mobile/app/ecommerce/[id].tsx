...
import type { EcommerceRecord, AssortmentItem, ChildBoxWithProduct, CartonMembership } from '../../types';
...
export default function EcommerceDetailScreen() {
  ...
  const onRefresh = async () => {
    setRefreshing(true);
    await Promise.all([recordQ.refetch(), assortmentQ.refetch(), cartonsQ.refetch()]);
    setRefreshing(false);
  };
  ...
  const scanCartonMutation = useApiMutation<{ added: number; cartonBarcode: string }, string>(
    (cartonBarcode) => ecommerceService.scanCarton({ ecommerce_record_id: id!, carton_barcode: cartonBarcode }),
    {
      successMessage: 'Carton added',
      invalidateKeys: id ? [...ECOMMERCE_INVALIDATE_KEYS(id), ['ecommerce-cartons', id]] : [],
      onSuccess: () => {
        recordQ.refetch();
        cartonsQ.refetch();
      },
