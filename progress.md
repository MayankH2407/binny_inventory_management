...
**🖥️ DEVICE SHIFT — RESUME HERE (2026-07-23).** Work is moving to a new machine mid-flight. **⚠️ TRANSFER WARNING:** local `main` is at commit `45c03ab` (returns web feature) which is **NOT pushed to origin** (ahead by 1), AND there is substantial **uncommitted** work in the tree (all of Mobile M7 + the 4 held web files + `docs/qr-label-layouting-guide.md`). A fresh `git clone` on the new device would get NEITHER `45c03ab` NOR any uncommitted work. **To preserve everything: copy the ENTIRE project folder (working tree included) to the new device** — do NOT rely on a clone/pull. (Alternative: push `45c03ab` + commit the mobile work first, but the 4 held files + qr guide would still need carrying.)
