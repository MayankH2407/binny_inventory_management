...
- **⚠️ SAME LATENT BUG, NOT YET FIXED, in other dropdowns** (4 more `getAll({limit:200})`): the **Child Boxes list Product filter** (`child-boxes/page.tsx:84` → product-based, will also show only Duke/Rocky) and three **customer** dropdowns (`dispatch`, `reports`, `samples/create` — lower risk unless customers > 200). Proposed to fix these too.
- **TEST still trails LIVE:** TEST build #1 (the 3-fix bundle, WITHOUT the article fix) was mid-build; TEST needs the article fix too (a follow-up build). LIVE is now ahead of TEST on these frontend fixes.
