...
- **TEST deploy — CURRENT EXECUTION (as of this entry):** ✅ committed `5a49766` (+ progress commit); ✅ synced `backend/src`+`backend/migrations`+`frontend/src`+`progress.md` to `/opt/binny` on `srv1409601` (clean-slate `rm -rf backend/src frontend/src`; migration file + new service headers confirmed landed); 🔄 building `binny-backend`+`binny-frontend` detached → `/tmp/binny-legacy-build-20260617.log` (frontend `next build` in progress on the loaded host). **STILL PENDING:** (1) `up -d binny-backend binny-frontend` after `BUILD_EXIT=0` (watch `:latest` lag — confirm running image == `:latest`); (2) **`docker compose -f docker-compose.prod.yml exec -T binny-backend npm run migrate:up`** to apply `20260616120001` (pgmigrations does NOT yet have it — verified); (3) verify on test portal (upload new-format sample → detail card + label render, assortment grid omitted); (4) hand to client for UAT.
- **LIVE deferred** → after client UAT. Live also needs `migrate:up` for `20260616120001`. Git: 7 commits ahead of `origin/main`, not pushed.
