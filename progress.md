...
**🚀🚀🚀 BIG BATCH LIVE DEPLOY (2026-08-20): Returns + Dispatch return-status + Samples redesign + E-commerce pool redesign + size_from/size_to fix — ALL DEPLOYED TO LIVE & VERIFIED. Client had approved all pending features except the child-box label Variant B A/B test (untouched, still TEST-only).** Opus planned (two independent passes plus an adversarial pre-mortem, cross-checked against each other and against live read-only queries on LIVE); Sonnet executed.

**Scope decisions made this session:** user initially wanted Samples held back from this deploy — checked and found it isn't separable from the uncommitted e-commerce work (6 shared files, interleaved inside the same functions: `dispatch.service.ts`'s new product-summary LATERAL literally queries `sample_carton_mapping`; `dispatch.schema.ts` has `sample_scope` beside `ecommerce_pool`; `constants.ts` shares an enum; plus `csvExport.service.ts`, `dispatch/page.tsx`, `types/index.ts`) — excluding it would mean hand-reverting committed hunks out of functions the e-commerce redesign rewrote, with no TEST re-pass to catch a mistake. User accepted including it once shown the alternative was riskier, not safer. Deploy route: straight-to-LIVE (both were already TEST-verified), no re-verification pass on TEST.

**Git (local, before any box was touched):** committed the previously-uncommitted e-commerce redesign as `bf65769` (26 files incl. the 2 untracked migrations) and the size fix as `119354e` — both had been sitting in the working tree since the 2026-08-12 TEST deploy. The one genuinely mixed file, `reports/page.tsx`, had **10 SearchableSelect hunks and ~23 e-commerce hunks interleaved** (not the 6-hunk estimate from a line-number-only read — the real hunk boundaries only became clear after LF-normalizing the file, since it was CRLF in the working tree vs LF in `HEAD` and a `-w`/`-b` diff **looks** right but can't actually be `git apply`'d until normalized). Extracted the exact SearchableSelect-only hunks into a standalone reverse patch, applied with `git apply -R --unidiff-zero`, verified with explicit greps (zero trace of `SearchableSelect`/`fetchCustomerOptions`/`useCallback`/`fetchOptions`; original customer-loading code restored; the e-commerce rewrite's `summary.dispatch_count`/`summary.pool.total_boxes`/`row.dispatch_id` all present) before `tsc --noEmit`. Also reverted `child-boxes/page.tsx` to `HEAD` (the actual SearchableSelect file) but **kept** `child-boxes/generate/page.tsx` — verified directly it has zero `SearchableSelect` references, its only change is an independent, already-safe distinct-articles query rewrite (one of the planning agents wrongly grouped it with the SearchableSelect exclusion; caught by reading the diff directly before acting on it).

**Found during pre-deploy audit, unrelated to any of the 4 features, fixed as its own commit (`4bf78e0`):** `backend/src/utils/autoSeed.ts` ran unconditionally on every backend boot and silently reset the admin password back to the public default (`Admin@123`) any time the stored hash didn't match — not just on real corruption. Confirmed directly against LIVE that this wasn't hypothetical: `admin@binny.com`'s `updated_at` was `2026-08-18`, 20 days after the backend's last restart, meaning the client had genuinely rotated the password since — and this deploy's own backend restart would have silently wiped it back to a publicly-known value. Fixed to never touch an existing admin row's password (only creates the row if missing); verified post-deploy that `updated_at` was still `2026-08-18`, unchanged by the restart.

**Label handled via a throwaway integration branch, not a post-sync restore:** LIVE's `childBoxLabel.ts` is byte-identical to branch tip `5688cae` (Variant A, confirmed by md5 `3ff4237775eab69800c41089c7fff847`) — `main`'s copy matches neither running A/B variant. Rather than sync-then-restore (which has an ordering trap: if the restore lands after `next build` already baked the wrong file in, the source on disk looks right and only a dist-grep would catch it), created a local-only `deploy-live-20260820` branch off the 3 commits above, `git checkout 5688cae -- frontend/src/lib/childBoxLabel.ts` on that branch only, committed, and `git archive`'d from there. `main` itself was never touched by the label pin and stays at the 3 real commits — confirmed clean before switching back. **This branch must never be merged** — the label A/B test is still undecided.

**Pre-flight found a real, if small, operational wrinkle and got explicit user sign-off before proceeding:** LIVE had 4 existing e-commerce records (3 `ACTIVE`, 1 `CLOSED`, 0 dispatched, 5 box mappings) whose old create/add-box/close workflow the redesign removes. Confirmed their boxes are not stranded (the pool query has no exclusion for pre-existing `ecommerce_record_id`, and the frontend tags them "Legacy `<barcode>`"), but any in-progress work on them via the old screens ends at cutover. User accepted given the small count.

**Migration mechanics — one real blocker found and worked around:** `node-pg-migrate` 7.9.1 defaults `checkOrder: true`. LIVE had applied `20260725100001` (dispatch timestamp) but never the earlier `20260721100001` (Returns), so a plain `migrate:up` throws `"Not run migration 20260721100001...is preceding already run migration 20260725100001..."` before touching any table. Harmless in effect (no real dependency between the two files) but required `--no-check-order` on every invocation. Ran a dry-run first (`--dry-run`, genuinely non-mutating — skips every `db.query` including the ledger insert) to confirm exactly the 4 expected migrations in the expected order, then ran for real. `20260812100002`'s `source_type` backfill and historical-mapping backfill — the only migration with real data-shape risk — were confirmed harmless against LIVE's actual data before running: 0 all-NULL-FK rows, 0 rows would violate the new constraint, 0 dispatch rows referenced `ecommerce_record_id` at all (so the historical backfill matched 0 rows — a complete non-event on this box), and metadata coverage for `return_status` was 6998/6998 (no legacy-dispatch gap to worry about).

**Deploy mechanics:** DB dump (`backup-pre-3module-20260820.dump`, 122MB, custom format) taken and **restore-tested into a scratch database** (row counts matched exactly: 6998 dispatch_records, 573,485 child_boxes) before trusting it as the rollback path — `migrate:down` was confirmed NOT to be a clean inverse (two enum additions can't be dropped; `ecommerce_record_id` stays nullable by design in the `down`), so the dump is the real rollback, not the migration system. `.env` hashed and backed up separately, never touched. Synced via `git archive` from the integration branch (not `tar` of a working tree) — emits only tracked committed content at LF, which excludes `mobile/`, QA artifacts, and CRLF noise by construction. Clean-slated `backend/src`/`frontend/src` first (required: this deploy deletes `ecommerce/create/page.tsx` and `tar` never deletes). Built all three images **serially and fully before recreating any container** (`binny-backend` → `binny-frontend` → `binny-frontend-root`, ~13 min total, each with `--env-file .env` so the `NEXT_PUBLIC_*` caps baked in) — deliberately avoiding a build-then-recreate-per-service interleaving that could have left the canonical `binnyfootwear.basiq360.com` portal on stale code against the new schema if a later build had stalled. Recreated all three together via `up -d --no-deps` (never touches `binny-db`, confirmed unchanged — created `2026-07-03`, uptime unaffected). `binny-nginx` (the shared, non-compose-managed HTTP ingress for ~15 other clients on this box) untouched throughout.

**Verified (unauthenticated only — LIVE admin creds remain client-rotated, so no authed API calls or UI driving; every check below is health/DB/dist-grep):** `pgmigrations` 39→43, exactly the 4 expected names; `return_records`/`return_items` tables present; `sample_box_mapping.source_master_carton_id` present; `chk_dispatch_source_exactly_one` replaced by `chk_dispatch_source`+`chk_dispatch_source_type`; 4 new pool indexes present; `source_type` backfilled 6998/6998 to `MASTER_CARTON`, 0 NULLs; backend dist confirms the 6 old e-commerce endpoints (`addBoxToEcommerce` etc.) gone and the pool functions (`getEcommercePool`/`addToEcommercePool`/`unpackCartonInEcommercePool`) present, `samples:update` guard count 8 (was 5), size fix present (3 `input.size_from` hits, was 2); both frontends confirm `Fully Returned`/`Ready to dispatch`/`E-commerce Area` present, `ecommerce/create` route gone, zero `fetchCustomerOptions` leakage (confirms the SearchableSelect exclusion held), label markers correct (`fitFillGroup`+`combined-row` present, `50mm 60mm` absent — Variant A survived); caps 1500/2000 confirmed both in backend `printenv` and baked into both frontends; health 200 on all 3 URLs (canonical + hstgr fallback + api); running image IDs == `:latest` on all three services; `surveydesk` (the coexisting tenant sharing this box's nginx) unaffected, still 200.

**Client comms still owed (not yet sent):** (1) Returns is Admin-only by default — grant `returns:create`/`returns:read` via Role Manager for other roles if wanted; everyone with a live session needs to log out/back in for updated permissions to take effect (cached client-side at login). (2) Samples redesign: partial carton take-out, scoped partial dispatch, friendly status labels, 3 pre-existing bugs fixed; `samples:update` now needed in more places — only Admin and "ECom Supervisor" currently hold it. (3) E-commerce is the biggest behavioural change: "create a named record, then scan into it" is gone; staff scan directly into an "E-commerce Area" pool; marketplace/order-ref/etc. now captured at Dispatch time instead; a pooled carton can be Unpacked into individually-dispatchable loose boxes; the 4 pre-existing records surface automatically in the pool tagged "Legacy `<barcode>`" and are visible read-only under History; `ecommerce:delete` (not `update`) is now needed to remove a pool item. (4) Inventory size-group display fix — going-forward only, no backfill for existing blank rows. (5) Pre-existing, NOT fixed here: Reports ▸ Samples tab crashes (`by_status` shape mismatch, unrelated to this deploy, was already broken); `getDispatchById` still doesn't join `customers` (shows "Walk-in / No Customer" even when set — same gap flagged earlier this project, still outstanding). (6) Mobile app's e-commerce screens will 404 against the new API — Mobile M7 (which already added mobile Returns support) remains uncommitted and un-built-to-APK, unaffected either way.

**Not yet done:** `main` (now at `4bf78e0`, 3 new commits: `119354e`/`bf65769`/`4bf78e0`) is 18 commits ahead of `origin/main` — push held pending explicit user go-ahead (separate shared-state action from the deploy itself). `deploy-live-20260820` integration branch still exists locally, unmerged, unpushed — safe to delete once the deploy is confirmed stable, but not urgent. `docs/live-deploy-checklist.md`, `HANDOFF.md`, `ONBOARDING.md`, and the client-facing `docs/project-brief.md`/`docs/implementation-plan.md` status tables still need updating to reflect this deploy (in progress this session).

---

**🚀 SAMPLES MODULE REDESIGN: DEPLOYED TO TEST (2026-08-01) & VERIFIED END-TO-END — now also DEPLOYED TO LIVE (2026-08-20), see entry above.** Committed `f23435f` (redesign) + `61bb18b` (fixes below). Backup taken first (`/opt/binny/backup-pre-samples-redesign-2026-07-31.sql`, 122MB). Deployed via surgical single-file sync (not a directory clean-slate — TEST's frontend is still on the child-box-label A/B test's Variant B branch, confirmed untouched post-deploy: `60mm 50mm` marker still present in the built bundle, `combined-row` absent). Backend + frontend rebuilt with `docker-compose.prod.yml` (⚠️ nearly built with the wrong compose file first — this box runs several other clients' stacks side by side and a bare `docker compose build/up` with no `-f` picked a different, dev-oriented compose file, spinning up parallel `binny_backend`/`binny_frontend`/`binny_postgres` containers with a **fresh, empty** Postgres volume; caught before anything started serving traffic — real `binny-db` still held port 5432 so the impostor stack failed to come up — removed the stray containers/network/volume/images, nothing was lost, real stack never stopped). Migration `20260731100001` applied cleanly (only pending one; `Can't determine timestamp` lines are the usual harmless node-pg-migrate quirk seen on every migration in this project). **Two real bugs surfaced during verification, both fixed and redeployed before sign-off:** (1) `next build`'s lint pass (stricter than a bare `eslint <file>` run, which uses a different ruleset than `next/core-web-vitals`) caught 2 unescaped-apostrophe `react/no-unescaped-entities` errors in `dispatch/page.tsx` and `samples/create/page.tsx` — escaped both. (2) A real logic bug: `takeBoxOutOfCartonAllocation` called `assertFootAvailable`, whose box-status gate rejects `PACKED` boxes — but every box being pulled out of a carton allocation starts `PACKED` by definition, so the new take-out-boxes endpoint 404'd on every real attempt. Split the function into `assertNoConflictingFoot` (foot-vs-foot conflicts only) and kept `assertFootAvailable` (adds the status gate) for the call sites where it's correct (adding a not-yet-sampled box); the carton take-out path now uses the former. **Full functional pass via direct API calls against TEST** (admin creds still valid there, unlike LIVE): confirmed `customer_firm_name` no longer blank on the list; took 1 box out of a real 48-box carton allocation (carton→47, `taken_out_count`→1, box flipped to loose/`SAMPLE` with `source_master_carton_id` set); `set-box-foot` PAIR→LEFT on that box; `remove-box` via `mapping_id` (the original core bug) correctly dropped `child_count` 48→47 and freed the box; `remove-carton` released the remaining 47-box carton back to stock untouched (`CLOSED`, `child_count` still 47) and the sample fell back to `CREATED`/0; creating a sample with no `name` produced `"QA Recipient · 2026-08-01"`; a 3-loose-box sample scoped-dispatched down to 1 box correctly marked the dispatched box `DISPATCHED`, freed the other 2 back to `FREE`, and stamped the dispatch record `metadata: {scoped:true, child_box_count:1, released_box_count:2}`. Playwright confirmed the friendly status labels render correctly on both the list page (badges + filter dropdown: Empty/Open/Ready to dispatch/Sent) and the detail page, and the merged Contents section with a real pre-existing foot-split box ("Left shoe only" chip + undo). Left several clearly-artificial test records on TEST (`SR3HK91S`, `SR176GTH`, boxes `CB52YD4H`/`CBZGJ4GG`/`CB0ETRRV` cycled through FREE→SAMPLE→FREE/DISPATCHED) — low risk, same precedent as prior TEST verification passes. **Not yet deployed to LIVE** — this is a real functional/DB-schema change (new migration, new endpoints, rewritten create/detail pages), not a low-risk asset swap, so it holds for explicit client UAT sign-off on TEST before any LIVE push, per the project's standard gate.

**🛒 E-COMMERCE MODULE STRUCTURAL REDESIGN (2026-08-10, deployed to TEST 2026-08-12, deployed to LIVE 2026-08-20) — DEPLOYED & VERIFIED ON BOTH. Opus plan / 2 Sonnet agents execute (backend then frontend). Committed as `bf65769` on 2026-08-20 (see the big-batch entry above for the LIVE deploy).**

**TEST deploy (2026-08-12):** DB backed up first (`/opt/binny/backup-pre-ecommerce-redesign-2026-08-10.sql`, 127MB). Deployed straight from the uncommitted local working tree (no git commit made — the working tree had 3 unrelated, still-undecided changes tangled in with this work that had to be surgically excluded, not just left uncommitted): `backend/src/services/product.service.ts` (the still-pending `size_from`/`size_to` fix), `frontend/src/lib/childBoxLabel.ts` (TEST's already-deployed 50x60mm sticker fix — a wholesale sync would have reverted it), and `frontend/src/app/(dashboard)/child-boxes/{page.tsx,generate/page.tsx}` (the still-undecided SearchableSelect perf change). The trickiest case: `reports/page.tsx` has BOTH the (wanted) E-commerce-tab rewrite AND the (unwanted) SearchableSelect change interleaved in the *same file* — reconstructed a clean copy by diffing against TEST's actual current baseline and manually reverting only the SearchableSelect/`fetchCustomerOptions` hunks, verified `tsc`-clean standalone, and confirmed zero trace of the excluded change before shipping. Synced via `tar --exclude=...` (additive, not a clean-slate wipe — the only real deletion was one directory, `ecommerce/create/`, removed explicitly by name) + the reconstructed `reports/page.tsx` dropped in separately. Learned mid-deploy that `backend/migrations` is baked into the Docker image at build time, not live-mounted — `migrate:up` silently reported "No migrations to run!" against the old image before the rebuild; correct order is build → recreate → migrate, not migrate → build.

**Verified on TEST with real data, both via API and live UI:** the pre-existing legacy e-commerce data already on TEST (1 `ACTIVE` + 1 `CREATED` record, 1 box) surfaced in the new pool automatically with zero rows rewritten, exactly per the migration's design (confirmed both via `pool/summary` counts before/after and visually — the pool table shows the real legacy box with a "Legacy ECC18M9A" provenance tag, and the History tab correctly lists both real legacy records read-only). Full functional loop run with clearly-artificial QA data (product `QAECOM01`, boxes prefixed for cleanup): packed carton → scanned into pool → confirmed the packed-box rejection names the correct carton → scanned the carton in → unpacked it (boxes landed `ECOMMERCE`, not `FREE`, confirmed via pool counts) → dispatched 2 boxes with Marketplace/Order Reference/etc. → confirmed `source_label`, product summary, and box count all correct on the resulting dispatch → partial return → `return_status` flipped to `partial` correctly. Visually confirmed all three key screens on the real TEST portal: E-commerce Area (scan panel + stat cards + pool table with Remove actions), History tab (read-only, real legacy records, amber banner), and Dispatch's E-commerce tab (multi-scan list + the new optional field group, mirroring the Master Carton tab exactly). Health 200, both containers rebuilt+recreated, sticker fix (`50mm 60mm` marker) confirmed still present in the deployed frontend bundle — unaffected by this deploy.

**Scope (confirmed with client over 2 rounds of clarifying questions):** e-commerce stops being "create a named record (name/marketplace/order ref/etc.), then scan boxes into it." Now: (1) staff scan an existing, non-dispatched Master Carton OR Child Box barcode directly in the E-commerce module — that single action moves the item (and everything inside it, if a carton) from main inventory into a separate "E-commerce Area" pool, with no naming/marketplace captured at scan-in time; (2) marketplace/order-reference/listing-SKU/reference-name/order-date move entirely to DISPATCH time, captured on the Dispatch module's E-commerce tab; (3) dispatch happens ONLY from Dispatch's E-commerce tab, scanning ANY box/carton currently in the pool into a removable list (mirrors the Master Carton tab's existing scan-multiple-then-submit-once UX), one dispatch per submit; (4) **new requirement surfaced during planning**: a pooled carton can be "Unpacked" into individually-dispatchable loose boxes that stay committed to e-commerce (not sent back to main stock) — rejected the Samples-module's bespoke partial-pull mechanic in favour of reusing the existing "Full Unpack" pattern with a different landing status.

**Planning (2 Opus passes):** first pass produced the full technical plan and surfaced real cross-cutting risk the ask alone didn't reveal — `return.service.ts` (3 lookup sites keyed off the old per-record FK), `csvExport.service.ts` (same), `report.service.ts` (the e-commerce report is 100% record-based, would need a full rewrite), a claimed-but-actually-already-fixed pack/unpack safety gap, and confirmation the mobile Expo app's e-commerce screens will 404 and are accepted as out of scope. Second pass designed the Unpack mechanic in full (new `CARTON_UNPACKED_TO_ECOM_POOL` transaction type — deliberately distinct from `CARTON_UNECOMMERCED` since the stock outcome is the opposite direction; not reversible in one step, by design — recovery path is Remove-then-repack) and caught 3 corrections to its own first pass before finalizing: the pack/unpack guard it thought was missing already existed; `dispatch/page.tsx` had no real held changes (only `reports/page.tsx` did — a Samples-tab filter); and `CARTON_UNECOMMERCED` existed in the enum but was unused anywhere in the codebase.

**Backend (Sonnet #1):** 2 migrations (`20260812100001` enum value, `20260812100002` — nullable `ecommerce_record_id` on both mapping tables, `dispatch_record_id`+`source_master_carton_id` added, `dispatch_records` gains `source_type` discriminator + `reference_name`/`marketplace`/`order_reference`/`listing_sku`/`order_date`, backfills `source_type` for all existing rows, replaces the old "exactly one FK" CHECK, backfills historical e-commerce dispatch→mapping links so legacy dispatches need no special-casing anywhere downstream). Deleted the 6 record-mutation functions/endpoints (`createEcommerce`, `addBox`, `scanCarton`, `removeBox`, `close`, `fullUnpack`); added 6 pool functions/endpoints (`scan`, `unpack-carton`, `remove`, `pool` list, `pool/summary`, `pool/lookup/:barcode`) — RBAC: scan/unpack on `ecommerce:update`, remove on `ecommerce:delete` (the one pool action that returns stock to main inventory). Rewrote `dispatch.service.ts`'s e-commerce path to accept an item array instead of one record id, fixed a real pre-existing gap while in there (the product-summary query was missing 2 of its 5 needed branches — sample-via-carton and both e-commerce branches — so `getDispatchById` showed a blank Product card for any whole-carton sample/e-commerce dispatch, confirmed independently by the frontend agent later too). Fixed the same root-cause bug in 5 places across `return.service.ts` (2 of which were pre-existing bugs unrelated to this redesign — a carton shipped via sample/e-commerce never resolved its origin dispatch for Returns), plus 2 more spots the plan didn't explicitly name but had the identical bug (`exportReturnCSV`, `getReturnById`'s per-item label) — fixed those too as a flagged deviation. Verified: `tsc` clean, migration applied + schema/constraint/backfill confirmed via `psql`, full manual smoke test via curl (scan/reject/unpack/dispatch/return/CSV/report, including the exact rejection wording for a packed-box-inside-a-different-carton scan), existing 15-test backend suite green before and after.

**Frontend (Sonnet #2):** `/ecommerce` rewritten as two tabs — "E-commerce Area" (scan panel, 4 stat cards, pool table with per-row Unpack/Remove + confirm modals, permission-gated) and "History" (old record list, fully read-only, amber banner) — replacing the old create/list/detail pages (detail page stripped of every mutating action, create page deleted). Dispatch page's E-commerce tab rebuilt to mirror the Master Carton tab's multi-scan-into-a-list UX exactly, plus a new optional "E-commerce Details" field group. Dispatches list/detail surface the new fields (marketplace/order-ref sub-line, a 4th "E-commerce Details" summary card). Reports page's E-commerce tab rewritten to the new dispatch-based shape (only that tab's code touched — confirmed via `git diff -w` that the pre-existing held Samples-tab filter change in the same file survived byte-for-byte). Verified: `tsc` + `eslint` clean, `mobile/` confirmed untouched, and a full live Playwright pass against the real local backend — scanned a loose box and a carton in, triggered the packed-box rejection (named the correct carton), Unpacked the pooled carton into 3 individually-removable boxes, dispatched 2 of them with Marketplace=Amazon/Order Reference=AMZ-ORD-88123 (removed-and-re-added one first to prove the list interaction), confirmed the resulting `/dispatches` row and detail page render correctly (including a populated Product card, thanks to the backend fix above), confirmed the pool shrank by exactly 2, confirmed History tab and the stripped detail page show zero mutating actions, confirmed Reports' E-commerce tab has no status filter and doesn't crash.

**Bugs found, correctly left unfixed (out of scope for this task, flagged for later):** (1) `getDispatchById` (single-record fetch, used by the dispatch detail page) never joins `customers` — shows "Walk-in / No Customer" even when one was set. **This is the same gap identified earlier in this session as an approved-but-not-yet-implemented Dispatch UI fix** (see the size_group/dispatch-UI work above) — still outstanding, now confirmed independently a second time. Affects every dispatch source type, not just e-commerce. (2) The Reports page's Samples tab crashes (`Cannot convert undefined or null to object` on `by_status`) because its backend endpoint no longer returns that shape — unrelated to this redesign, reproduces regardless of the e-commerce changes, correctly left alone since `reports/page.tsx`'s Samples-tab code is the one section this work was explicitly barred from touching.

**Not yet done (superseded 2026-08-20):** ~~nothing committed to git; no decision yet on TEST/LIVE deploy~~ — client approved, committed `bf65769`, and deployed to LIVE 2026-08-20. See the big-batch entry above.

**🚀 DISPATCH MODULE UI FIXES (2026-08-10): filter-bar single-line + multi-expand customer groups — DEPLOYED TO TEST & LIVE & VERIFIED.** `dispatches/page.tsx`, commit `2a8c430` on `main`. (1) **Filter bar wrap bug:** the From/To date `Input`s were passed a width `className` directly, but `Input`'s outer wrapper div is hardcoded `w-full` and ignores it — only the inner `<input>` respected the width, so the wrapper still claimed the full row and forced every filter onto its own line even on a wide desktop screen. Fixed by wrapping both date `Input`s in sized `<div>`s (matching the pattern already used for the two `Select`s next to them). Also switched the outer container from `sm:flex-row` to `lg:flex-row` and gave the search box `min-w-[160px]` so it shrinks to make room instead of wrapping — search + From + To + Source Type + Returns now sit on one line from `lg:` up. (2) **Customer-group accordion:** `expandedCustomer` was a single `string | null`, so only one customer group could ever be open at a time and every group started collapsed — meaning a search result with just 1-2 parties looked like an empty page until you clicked each header. Changed to a `Set<string>` defaulting to every currently-loaded group expanded (a page is already capped at `PAGE_SIZE`=20 records, so there's no "too many to show" case that benefits from collapsing by default), while still respecting a manual collapse within the same result set. **Deployed to TEST** via the same surgical sync technique as before (dispatch fix from `main` + the sticker fix below from its own branch, two separate files, no directory sync); health 200, both fix markers (`min-w-[160px]`, `lg:flex-nowrap`) confirmed in the built bundle, and a live Playwright pass against real TEST data confirmed both fixes render correctly (filter bar on one line at 1440px and at tablet width; both customer groups open with full records, no dead space). **Deployed to LIVE, dispatch-only** (explicitly NOT the sticker fix, since LIVE runs a different label variant entirely) — but first discovered LIVE's `dispatches/page.tsx` is missing the entire "Return Status" filter/badge feature (built 2026-07-22) that TEST/`main` already have, so a straight `main`-sync would have shipped that unrelated, never-approved-for-LIVE feature as a side effect. Hand-patched LIVE's actual current file with just the two targeted diffs instead, verified the diff contained *only* those two changes (no `returnStatusFilter` anywhere), `tsc`-checked standalone before shipping. Rebuilt + recreated both `binny-frontend` and `binny-frontend-root`; health 200 both LIVE URLs, fix marker present in both dist, `Fully Returned` marker absent (confirms the unrelated feature didn't leak). No backend/DB changes either environment, no migration.

**🏷️ CHILD-BOX LABEL VARIANT B DIMENSION CORRECTION (2026-08-10) — DEPLOYED TO TEST ONLY.** Client caught that the 2026-07-29 Variant B spec had length/width backwards: 60mm is the *length* (feed direction), 50mm is the *width* (across-roll), not the reverse as built. `frontend/src/lib/childBoxLabel.ts` on branch `label-ab/variant-b-60x50`, commit `474e10b`. Swapped `@page`/`.row`/`.label` from `width:60mm;height:50mm` to `width:50mm;height:60mm`, content box 58x48mm → 48x58mm, and rebalanced the internal grid rather than just resizing it: content width shrank 58→48mm (text-detail column absorbs the loss, 34→25mm; size+QR column stays 23mm since QR scannability shouldn't shrink) and content height grew 48→58mm (every row height scaled up ~1.234x — article 9→11mm, colour 7→9mm, mrp 10→12mm, small rows 5→6mm each, footer 11→14mm). **Side benefit:** width going back to 50mm means 2-up printing on the existing 100mm roll is dimensionally restored (2×50mm=100mm) — not re-enabled here (kept at 1-per-page to stay a pure dimension-correction pass), but the ~62mm-stock requirement Variant B previously needed is gone if 2-up is wanted later; TSC driver media size still needs a change (100x50→100x60) before real printing either way. Verified locally (reproduced the exact swap, checked row/column arithmetic sums to the new content box exactly) then deployed via the same surgical single-file sync as always for this A/B test (git worktree for the branch commit, not the main working tree); `tsc` clean; health 200, `50mm 60mm` marker confirmed in the deployed TEST bundle. **NOT on LIVE** — LIVE runs Variant A (the merge-content-mfg design), which this branch never touches.

**🐛 INVENTORY BUG FOUND (2026-08-10): size-range bulk-create silently drops `size_from`/`size_to` — FIXED, committed `119354e`, DEPLOYED TO LIVE 2026-08-20 (see big-batch entry above).** Client report: the Inventory drill-down's `size_group` level showed one umbrella total (e.g. "42 cartons") for an article with no way to see the breakdown by actual size range (e.g. how many are 6-9 vs 7-10). Root cause: `backend/src/services/product.service.ts` `bulkCreateProductsBySizeRange()` (lines ~423-431) hardcoded `size_from`/`size_to` to literal `null, null` in the INSERT, instead of using the validated `input.size_from`/`input.size_to` — every product ever created via this endpoint (the "Multi-size batch generation" / size-range bulk-create feature, live since ~April 22) got a blank size_group label, so multiple distinct size-range batches under the same article/colour all collapsed into one indistinguishable bucket. The other two creation paths (single create, CSV upload) were already correct — only this one function had the bug. Fix: pass through the real values. Reproduced and verified the fix locally: two batches (6-9 and 7-10) under one article+colour now show as two separate `size_group` entries via the breakdown API, matching the client's expected semantics exactly (an exact-match group, not a range-overlap bucket — "6-10" only ever means the literal batch created with that exact range, never "contains any size 6 through 10"). **Decision: no backfill for existing null rows** on TEST/LIVE — going-forward-only, matching how prior casing-normalization fixes were handled in this project. Still sitting uncommitted as of 2026-08-10; not deployed anywhere, so the display gap persists on TEST/LIVE until this ships.

**📌 Ops note (2026-08-10):** the TEST/LIVE SSH deploy keys documented in `ONBOARDING.md` (`~/.ssh/id_ed25519`) are stale on the current (post-transfer) machine — that bare key doesn't exist here. Working mapping, verified today: **TEST** (`srv1409601.hstgr.cloud`, `/opt/binny`) via the `alstone-vps` SSH config alias; **LIVE** (`srv1689976.hstgr.cloud` / `binnyfootwear.basiq360.com`, `/opt/binny`) via the `surveydesk-hostinger` alias — both currently resolve to the same underlying key file (`~/.ssh/id_ed25519_alstone_vps`) per `~/.ssh/config`. Use the aliases, not a hand-typed `-i` path.

<details>
<summary>Original build notes (2026-07-31, pre-deploy)</summary>

**🛠️ SAMPLES MODULE REDESIGN (2026-07-31): full workflow rebuild, built + locally verified, NOT committed, NOT deployed.** Client feedback: Samples wasn't in active use because even staff couldn't follow how the scan-based flow was supposed to work — real usage is 1 piece, a few pairs, or occasionally bulk, never predictably whole-carton. Traced the existing code end-to-end first (no design changes until the actual mechanics were understood): confirmed foot-splitting (`sample_box_mapping.foot`, `assertFootAvailable`) and carton-membership (`sample_carton_mapping`, pure link table, carton stays packed) were both real and load-bearing, but 3 concrete bugs made the whole thing effectively unusable — (1) **remove-box never worked**: frontend sent `mapping_id` (a sample can hold both feet of one box as two separate mappings), backend only accepted `child_box_id`; (2) **full-unpack didn't release cartons**: whole-carton allocations stayed locked forever, `child_count` was hardcoded to 0 instead of recomputed; (3) **CSV dispatch export double-counted** boxes from inactive `carton_child_mapping`/`sample_carton_mapping`/`ecommerce_carton_mapping` rows (missing `is_active` filters). Fixed all three as a prerequisite, then rebuilt on top per client-confirmed direction (foot-split: keep but de-emphasize; carton-scan: keep, fix the dispatch gap, keep on create page behind a disclosure; partial-dispatch remainder: released back to general stock, not held for the same recipient; Close: kept, moved to an overflow menu). **Backend:** new migration `20260731100001_sample-carton-partial-allocation` (nullable `source_master_carton_id` on `sample_box_mapping` + 2 new transaction-type enum values, `CARTON_UNSAMPLED`/`CARTON_UNECOMMERCED`); `sample.service.ts` gained `recomputeSampleChildCount`, `takeBoxOutOfCartonAllocation` (pull specific boxes out of a whole-carton allocation without disturbing the rest), `releaseCartonFromSample`, `deactivateLooseMapping`, plus 3 new endpoints (`take-out-carton-boxes`, `remove-carton`, `set-box-foot`); `name` is now optional with an auto-generated default (`"<customer> · <date>"` / `"Sample <barcode>"`); fixed 2 pre-existing joins so the detail page's Print Label and the list page's customer column were no longer silently blank. `dispatch.service.ts` gained an optional `sample_scope` (partial dispatch: pick which boxes/cartons go, releases the rest back to stock in the same transaction). **Frontend:** Sample create page rebuilt — no more upfront foot-mode picker, every scan defaults to a full pair with a per-item "Send one shoe only" toggle; carton scanning moved behind a "Scan a whole carton instead" disclosure; recipient details collapsed into a "Who's it for?" panel. Sample detail page rebuilt — merged carton + loose-box contents into one list, carton groups collapsed by default showing "N of M boxes, reserved whole", a new take-boxes-out modal, overflow menu (Copy Barcode / Close / Empty Sample). Dispatch page's Scan Sample panel gained a checkbox review list (all-checked by default) with an amber warning when a partial selection will release boxes back to stock. Added friendly lifecycle labels throughout (CREATED→"Empty", ACTIVE→"Open", CLOSED→"Ready to dispatch", DISPATCHED→"Sent") replacing raw DB status strings on both the list and detail pages, incl. the status filter dropdown. **Verified:** `tsc --noEmit` clean on both backend and frontend app source (17 unrelated pre-existing `e2e/*.spec.ts` Playwright-type errors, untouched by this work); `eslint` clean on every changed file; backend `jest` suite runs but all 4 suites fail on `password authentication failed for user "binny_admin"` — this WSL2 environment has no working connection to this project's Postgres instance (pre-existing environment limitation, same failure mode regardless of these changes, not a regression). No Playwright render pass done yet (no live backend to point it at). **Not yet committed to git** (nothing in this redesign touched `main`) and **not yet deployed anywhere** — next step is tsc/lint re-check after any further edits, then commit, then TEST deploy + verification, then hold for explicit go-ahead before LIVE (this is a real functional/DB-schema change, not a low-risk asset swap — full UAT-gated process applies, unlike the recent label A/B test).

</details>

**🐛 FIX (2026-07-31): Sample label (`sampleLabel.ts`, built 2026-07-25) had a real layout-breaking bug — fixed.** Every field used a fixed font-size with no dynamic fit logic (unlike `childBoxLabel.ts`/`masterCartonLabel.ts`, which both already solved this via a `fit-box` + `fitFill`/`fitShrink` pattern). Confirmed via a Playwright-driven render with worst-case data: a long article name wrapped to 4 lines at a fixed 15pt, and since a CSS table row has no real max-height, the row simply grew past its intended 10mm and pushed everything below it out of the 50mm sticker — a real, reproducible defect, not cosmetic. Retrofit the same `fit-box`/`fitFill` treatment already proven on the other two labels: article/colour/size cells wrap+shrink to a fixed-height box, MRP uses a fixed-height flex column (main value grows to fill, caption stays put), Sample Date/recipient wrap within their own fixed-height box. Found and fixed one bug of my own making mid-verification: the Sample Date/recipient boxes were briefly a flex row (to vertically center), which split the bold "To:"/"Sample Date:" prefix from the wrapping text (flex items don't reflow across lines the way inline text does) — reverted to plain block flow, which is the correct tool when a bold prefix + wrapping value need to flow together like a normal paragraph. Verified: both typical and worst-case data (long article, 3-colour list, wide size range, big MRP, long recipient firm name) now render fully contained with zero overflow. `tsc` clean. **Deployed to both TEST and LIVE** (single-file surgical sync from `main`, NOT a full HEAD/directory sync — the childBoxLabel.ts A/B test below is still running on both boxes and a directory sync would have silently reverted whichever variant each box is running). Health 200 on all 3 URLs, LIVE caps intact, and dist-confirmed on every frontend: the sample-label fix (`article-cell` marker) landed AND each box's childBoxLabel A/B variant is untouched (LIVE still shows `combined-row`, TEST still shows `60mm 50mm`).

**🧪 CHILD-BOX LABEL A/B TEST (2026-07-29): two variants built + locally verified, NOT yet deployed.** Client wants to compare two label redesigns before picking one: **Variant A** (LIVE-bound) merges the "Content: XN (Y Pair)" and "Mfg & Mktd by..." rows into one 15mm combined-row (was 5mm + 10mm separately) — footprint stays 48x48mm, QR-cell rowspan drops 3→2 (still 20mm total, unchanged); content line itself doesn't grow (width-bound, not height-bound), mfg line gets ~12% more room. **Variant B** (TEST-bound) keeps the row structure identical (no merge) but resizes the whole sticker to 60x50mm stock (content box 58x48mm) — columns 27+20mm→34+23mm, article row 10mm→9mm (funds +1mm elsewhere), footer row 10mm→11mm, QR 15mm→15.5mm + caption 7pt→8pt, size-numeral fallback ladder 52/34/24/18→60/40/30/22pt; colour/MRP/packed-on/content rows and the 17mm size-cell stay unchanged. **Flagged consequence for B:** 2-up no longer fits the existing 100mm roll (2×60mm > 100mm) — prints 1-per-page now, needs new ~62mm label stock + a TSC driver media-size change before printing for real. Planned by an Opus agent (exact mm-level spec, git/deploy strategy, open-questions list — see that agent's full output if needed), executed by Sonnet. **Git strategy:** two branches off `main` (`8a71ef2`) that intentionally do NOT merge until the client picks a winner — `label-ab/variant-a-merge-content-mfg` (`db0ab63`) and `label-ab/variant-b-60x50` (`12191a8`), each touching only `frontend/src/lib/childBoxLabel.ts`. `main` itself is untouched and matches neither variant. **Verified locally** (no live backend available in this environment — Docker isn't wired into this WSL2 distro and local Postgres isn't this project's own instance): built a standalone static-HTML harness reproducing each variant's exact CSS/JS (same `fitFill`/`fitShrink` logic), driven by Playwright with both typical data (short article/colour/size) and worst-case data (long article name, long colour, `10.5K` size, `24N (12 Pair)` content) — both variants pass a zero-overflow check (`scrollWidth`/`scrollHeight` vs `clientWidth`/`clientHeight` on every fitted block) and look correct in screenshots. **DEPLOYED 2026-07-29: Variant A live on LIVE (both `binny-frontend` + `binny-frontend-root`), Variant B live on TEST.** Single-file surgical sync per branch (no migration, no backend/DB touch at all — lowest-risk deploy of the day). Health 200 on all 3 URLs (LIVE canonical + fallback, TEST), LIVE caps confirmed still 1500/2000 (unaffected by this change, sanity-checked anyway), and each box's dist confirmed serving the right variant (`combined-row` marker present on both LIVE frontends, `60mm 50mm` marker present on TEST's frontend). Client is confident A will win (B's new-stock/driver-reconfig hassle is a hard sell) but wants the live comparison anyway. Reconciliation plan (once decided): rebuild the losing box from `main`, plus revert TEST's label stock/driver to 50x50/100mm if B loses — not yet executed, no decision made yet. **Follow-up (2026-07-29, LIVE-only, `5688cae`):** Content+Mfg were being fit independently within the merged block, so they rendered at different sizes and jointly used well under 100% of the 13mm space (Content's short line stayed small regardless of available height). Client asked for a uniform size maximizing combined space use. Wrapped both in a shared `.combined-fit` container, made Content wrap too (was `nowrap`, which capped it below Mfg's potential size), and added `fitFillGroup()` — binary-searches ONE font size applied to every matched element, checked against their *combined* height (not each independently, like every other `fitFill` call on this label). Verified locally: both lines land on the identical size and jointly fill 100% of the block on both typical and worst-case data (was well short of that before). Deployed to LIVE only (TEST's Variant B has no merged block, nothing to change there) — both frontends rebuilt, health 200, caps intact, `fitFillGroup` confirmed in both frontends' dist.
