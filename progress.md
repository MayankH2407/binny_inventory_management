...
**June 20 work DEPLOYED TO TEST & VERIFIED** (commit `20fe77a` on local `main`, NOT yet pushed to origin; NOT on live). Legacy "Pairs per Carton" + SIZE FROM/TO split + legacy box-count display fix — see the June 20 entry below. Everything before `20fe77a` is committed + pushed (`origin/main` @ `35dbd8a`) + deployed on BOTH test and live. **Open: push `20fe77a` to origin; client UAT on test; then LIVE deploy** (needs both frontends rebuilt `--env-file .env` + `migrate:up 20260620120001`).
...
- **Next: push `20fe77a` to origin → client UAT on test → LIVE deploy** (both frontends rebuilt `--env-file .env` so 1500/2000 caps stay baked + **`migrate:up 20260620120001`** on live).
