...
**🔻 MOST RECENT (2026-07-17):** **Client colour-repeat bug FIXED on LIVE (data merge + isolated backend code deploy) & TEST (data only).** Root cause = colour casing splitting variants into duplicate rows. Commits `2406421` (distinct-articles prep) + `cdd5a24` (colour UPPERCASE + case-insensitive grouping; deployed to LIVE isolated via cherry-pick to avoid shipping un-UAT'd carton). Data script `scratchpad/colour-merge.sql`. See the "July 17, 2026" entry below. Open: colour code fix to TEST; 7 mixed-MRP MOGLI PLUS 02 groups need client price decision.
...
**TEST (`srv1409601`) — DATA done, CODE pending.** Backup `backup-pre-colour-merge-20260716.sql.gz` (25M). Merge committed: 705 casing groups → 698 merged, **7 skipped (mixed MRP: MOGLI PLUS 02 ₹117 vs ₹129 — needs client price decision)**, 3546 dup rows deleted, 4628 boxes repointed; casing_groups_remaining=0; invariant 125,927 boxes held. ⚠️ **Colour CODE fix NOT yet deployed to TEST** (would recur on new imports until then) — will land with the next TEST HEAD deploy (carton already on TEST, so HEAD deploy is clean there).

**Still open:** (a) deploy colour code fix to TEST; (b) 7 mixed-MRP MOGLI PLUS 02 groups need client to pick correct price; (c) broader same-casing duplicate-row dedupe (~397 LIVE / thousands TEST) remains a separate exercise; (d) FC-09 "6-10" numeric note couldn't be reproduced from data (its colours are now clean) — get client screenshot if it persists.
