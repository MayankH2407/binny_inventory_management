...
| 1.4 | August 20, 2026 | Section 18 updated: Phase 6 mods #1–#5 confirmed live. Returns Management (§18.7) moved from "awaiting UAT" to deployed+verified LIVE. Three new items added and deployed the same day: Samples Module Redesign (§18.8), E-commerce Module Redesign (§18.9, scan-to-pool model), Inventory Size-Group Display Fix (§18.10). Also fixed, unrelated: `autoSeed.ts` was silently resetting the production admin password to its public default on every backend restart — patched to never touch an existing admin row's password. |
...
## 18. Phase 6 — Post-QA Client Modifications (May–August 2026)

A set of client-requested enhancements made after Phase 1 go-live, tracked as numbered mods. Mods #1–#5 and the Returns/Samples/E-commerce work below are all **live in production** as of 2026-08-20. Day-by-day execution history lives in `progress.md`; this section is the technical summary.
...
### 18.7 Mod — Returns Management (NEW module, July 2026)
- Brings physically-returned stock back into sellable inventory. **Two entry points (per client rework):** (a) the **Returns module is blind-scan only** — scan an already-`DISPATCHED` child-box QR *or* master-carton barcode; the app fetches the item's origin dispatch + details and, on confirm, adds it back to inventory and writes a return entry; (b) **returning against a specific dispatch happens on the dispatch detail page** (`/dispatches/[id]`) — its returnable items are shown with checkboxes so the user picks which come back (partial allowed). An optional **"Reason for return"** remark is captured on both. **Returnable:** whole master **cartons** + loose **child boxes**, from **master-carton** & **e-commerce** dispatches. **Samples are out of scope** (rejected, not crashed). **Disposition:** everything goes straight back to sellable — **physical stock only** (no condition/damage tracking, no return value, no approval gate).
- **Key design:** everything shipped is already `DISPATCHED` and `carton_child_mapping` stays active through dispatch, so a return simply reverses status — **no new child/carton status enum** (reuses `FREE` / `PACKED`+`CLOSED`). Inventory queries already count `FREE`/`PACKED` and exclude `DISPATCHED`, so returned stock **re-enters inventory automatically** (`inventory.service` untouched). Purely additive to existing dispatch code paths.
- **State transitions:** loose **BOX** `DISPATCHED→FREE` + deactivate any active mapping (`carton_child_mapping`/`ecommerce_box_mapping`); whole **CARTON** `DISPATCHED→CLOSED` (`dispatched_at=NULL`), its `DISPATCHED` boxes `→PACKED`, `carton_child_mapping` kept active, active `ecommerce_carton_mapping`/`sample_carton_mapping` deactivated. Only `DISPATCHED` items are returnable (duplicate/re-return guard → 400).
- **Migration `20260721100001_create-returns.js`:** tables `return_records` (optional origin `dispatch_record_id`, `customer_id`, `returned_by`, `return_date`, **`reason`**, `notes`, `metadata`) + `return_items` (box-granularity ledger: `child_box_id`, `master_carton_id`, **`dispatch_record_id`** [per-item origin, resolved server-side even for blind scan], `item_type` BOX|CARTON w/ `chk_return_item_type`); adds `transaction_type` enum values `CHILD_RETURNED`/`CARTON_RETURNED` (down is a no-op — PG can't drop enum values); `trigger_set_updated_at` on `return_records`.
- **Backend:** `return.service.ts` (`lookupReturnable`, `getDispatchReturnableItems`, `resolveOriginDispatchId` helper, transactional `createReturn` w/ `FOR UPDATE` + dedupe by `UPPER(barcode)` + `CREATE_RETURN` audit, `getReturnById`, `getReturns`), `return.controller.ts`, `return.routes.ts` (registered `/returns`), Zod `return.schema.ts`, `exportReturnCSV` (17-col itemized incl. Reason + per-item origin). New **`returns` permission module (create/read)** in `permissions.ts`. Endpoints: `POST /returns`, `GET /returns`, `GET /returns/export`, `GET /returns/lookup/:barcode`, `GET /returns/dispatch/:id/items`, `GET /returns/:id`.
- **Frontend:** `returns/{page,create/page,[id]/page}.tsx` (list + Export CSV; **blind-scan-only** create + Reason; detail) + **NEW `dispatches/[id]/page.tsx`** (dispatch detail with a pick-items Return action + Reason) + `dispatches/page.tsx` rows open the detail page, `returns.service.ts`, ROUTES `DISPATCH_DETAIL` + NAV_ITEMS (icon `Undo2`), `useCan('returns:create')` gating.
- **Dispatch return-status visibility (2026-07-22):** the Dispatches list surfaces whether each dispatch is **not / partially / fully returned** without opening it — a computed `return_status` (`returned_box_count` from `return_items.dispatch_record_id` vs `total_box_count` from `dispatch_records.metadata.child_box_count`) shown as a per-record badge + "X of Y boxes returned", a **server-side return-status filter**, and a **customer-group roll-up**; the detail page marks already-returned items. No schema change (`getDispatches`/`getDispatchById` computed fields + filter; `getDispatchReturnableItems` per-item `returned`/`returned_at`).
- **Status:** committed `45c03ab`, deployed TEST 2026-07-22, **deployed + verified LIVE 2026-08-20** (as part of the combined batch in §18.8 below). RBAC note: the `returns` permission module is not granted to any role by default at launch — Admin reaches it via the hard-coded super-admin bypass in `authorizePermission`; other roles need an explicit Role Manager grant.

### 18.8 Mod — Samples Module Redesign (August 2026)

Client feedback: even staff couldn't reliably follow the original scan-based Samples flow, and real usage is almost never a whole carton — usually one piece or a handful of pairs. Traced the existing mechanics first (per-foot sample tracking via `sample_box_mapping.foot`, whole-carton allocation via `sample_carton_mapping`) and found 3 pre-existing bugs before touching the design: **remove-box** never worked (frontend sent `mapping_id`, backend only accepted `child_box_id`); **full-unpack didn't release cartons** (`child_count` was hardcoded to 0 instead of recomputed, so whole-carton allocations stayed locked forever); **CSV dispatch export double-counted** boxes from inactive mapping rows across three tables (missing `is_active` filters). Fixed all three, then rebuilt the workflow: every scan now defaults to a full pair with a per-item "send one shoe only" toggle (was an upfront foot-mode picker); carton scanning moved behind a "Scan a whole carton instead" disclosure; a new `takeBoxOutOfCartonAllocation` lets specific boxes be pulled out of a whole-carton allocation without disturbing the rest; partial dispatch releases un-selected items back to general stock in the same transaction (`sample_scope` on the dispatch schema); lifecycle stages get friendly labels (Empty/Open/Ready to dispatch/Sent) instead of raw status strings.

**Migration `20260731100001_sample-carton-partial-allocation.js`:** adds nullable `sample_box_mapping.source_master_carton_id` (provenance — which carton a taken-out box came from) + a partial index; adds `transaction_type` enum values `CARTON_UNSAMPLED`/`CARTON_UNECOMMERCED` (the latter used by the E-commerce redesign in §18.9, added here since both were designed together).

**Backend:** `sample.service.ts` gains `recomputeSampleChildCount`, `takeBoxOutOfCartonAllocation`, `releaseCartonFromSample`, `deactivateLooseMapping`, plus endpoints `take-out-carton-boxes`/`remove-carton`/`set-box-foot`; `name` is now optional with an auto-generated default. `dispatch.service.ts` gained the optional `sample_scope` partial-dispatch parameter.

**Frontend:** Sample create/detail pages rebuilt per the workflow above; Dispatch's Scan Sample panel gained a checkbox review list with an amber warning when a partial selection will release boxes back to stock; new `frontend/src/lib/sampleStatus.ts` for the friendly labels.

**Status:** committed `f23435f` + `61bb18b` (fixes found during TEST verification), deployed TEST 2026-08-01 and verified end-to-end (real 48-box carton take-out, foot-split, remove-box, scoped partial dispatch), **deployed + verified LIVE 2026-08-20**.

### 18.9 Mod — E-commerce Module Redesign (August 2026)

Replaces the prior "create a named e-commerce record, then scan boxes/cartons into it" model. New flow: staff scan an existing, non-dispatched Master Carton or Child Box barcode directly in the E-commerce module — one action moves the item (and its contents, if a carton) into a separate **"E-commerce Area" pool**, with no naming/marketplace captured at scan-in; marketplace/order-reference/listing-SKU/reference-name/order-date move entirely to **dispatch time**, captured on the Dispatch module's E-commerce tab; dispatch happens only from there, scanning any pool item into a removable list before one combined submit (mirrors the Master Carton tab's existing UX). New requirement surfaced during planning: a pooled **carton can be "Unpacked"** into individually-dispatchable loose boxes that stay committed to e-commerce rather than returning to main stock — a new `CARTON_UNPACKED_TO_ECOM_POOL` transaction type, deliberately distinct from `CARTON_UNECOMMERCED` since the stock direction is the opposite of returning a carton to main stock; not reversible in one step by design (recovery path is Remove-then-repack).

**Migrations:** `20260812100001_add-carton-unpacked-to-ecom-pool-transaction-type.js` (the enum value) + `20260812100002_ecommerce-pool-redesign.js` — makes `ecommerce_record_id` nullable on both `ecommerce_box_mapping`/`ecommerce_carton_mapping`, adds `dispatch_record_id`+`source_master_carton_id` to support pool-to-dispatch linkage, adds `dispatch_records.source_type` discriminator (backfilled from the existing per-record FKs, then `NOT NULL`) plus `reference_name`/`marketplace`/`order_reference`/`listing_sku`/`order_date`, replaces the old "exactly one FK set" CHECK with a `source_type`-aware one, and backfills historical e-commerce dispatch→mapping links so pre-existing e-commerce dispatches need no special-casing downstream.

**Backend:** deleted the 6 record-mutation endpoints (`createEcommerce`/`addBox`/`scanCarton`/`removeBox`/`close`/`fullUnpack`); added 6 pool endpoints (`scan`/`unpack-carton`/`remove`/`pool` list/`pool/summary`/`pool/lookup/:barcode`) — scan/unpack gated on `ecommerce:update`, remove on `ecommerce:delete` (the one pool action returning stock to main inventory, hence the stricter permission). Rewrote `dispatch.service.ts`'s e-commerce path to accept an item array instead of a single record id; while in there, fixed a pre-existing gap where `getDispatchById`'s product-summary query was missing 2 of its 5 needed branches (sample-via-carton and both e-commerce branches), leaving the dispatch detail page's Product card blank for any whole-carton sample/e-commerce dispatch. Fixed the same root-cause bug across 5 sites in `return.service.ts`/`csvExport.service.ts` (some pre-existing, unrelated to this redesign — a carton shipped via sample/e-commerce never resolved its origin dispatch for Returns).

**Frontend:** `/ecommerce` rewritten as two tabs — "E-commerce Area" (scan panel, stat cards, pool table with per-row Unpack/Remove) and "History" (the old record list, fully read-only) — replacing the old create/list/detail pages. Dispatch's E-commerce tab rebuilt to mirror the Master Carton tab's multi-scan UX plus a new optional field group. Reports' E-commerce tab rewritten to the new dispatch-based shape (no status filter — an e-commerce dispatch either happened or it didn't).

**Pre-existing bug found, correctly left unfixed (out of scope, flagged separately):** Reports ▸ Samples tab crashes (`by_status` shape mismatch between the frontend and its backend endpoint) — reproduces regardless of this redesign, and `reports/page.tsx`'s Samples-tab code was explicitly barred from this task's scope.

**Status:** built and verified end-to-end on TEST 2026-08-12 (real legacy data surfaced correctly in the new pool with zero rows rewritten; full scan→unpack→dispatch→return loop tested with QA data). Committed `bf65769` on 2026-08-20 (had been sitting uncommitted since the TEST deploy) and **deployed + verified LIVE the same day**, alongside Returns and the Samples redesign — the three could not be deployed independently since they share several rewritten files (`dispatch.service.ts`, `dispatch.schema.ts`, `constants.ts`, `csvExport.service.ts`).

**Found and fixed during the pre-LIVE-deploy audit, unrelated to any of the above:** `backend/src/utils/autoSeed.ts` ran unconditionally on every backend boot and silently reset the admin account's password back to the public default if it didn't bcrypt-match — not limited to actual corruption, so a deliberately rotated production password would be silently wiped on the next restart. Fixed to never touch an existing admin row's password (commit `4bf78e0`); confirmed post-deploy that LIVE's admin `updated_at` was unaffected by the restart.

### 18.10 Mod — Inventory Size-Group Display Fix (August 2026)

`bulkCreateProductsBySizeRange()` (the "Multi-size batch generation" bulk product-creation endpoint) hardcoded `size_from`/`size_to` to `NULL` in its INSERT instead of using the validated input values — every product created via this path since the feature's original release got a blank `size_group` label, so the Inventory drill-down showed one combined total per article/colour instead of separate entries per size-range batch created. The other two creation paths (single create, CSV upload) were already correct. Fixed to pass through the real values; **no backfill** for existing NULL rows, matching how prior going-forward-only data-normalization fixes were handled on this project. Committed `119354e`, **deployed + verified LIVE 2026-08-20**.
